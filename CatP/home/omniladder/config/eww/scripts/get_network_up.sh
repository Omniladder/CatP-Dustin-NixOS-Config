#!/bin/sh
# Get network upload speed from speed test in format "Up: ↑ X.XMB/s"
  ...
  if [ -n "$prev_line" ]; then
    ...
    if [ -n "$prev_time" ]; then
      ...
      if [ "$time_diff" -ge "$test_interval" ]; then
        ...
        # Return cached upload speed (field 1)
        upload_mbps=$(echo "$prev_line" | awk '{print $1}')
        if [ -n "$upload_mbps" ] && [ "$upload_mbps" != "0" ] && [ "$upload_mbps" != "0.0" ]; then
          echo "Up: ↑ ${upload_mbps}MB/s"
          exit 0
        fi
      fi
    fi
...
if [ "$run_test" = true ]; then
  ...
  if [ -f "$test_lock" ]; then
    ...
    if [ -f "$cache_file" ]; then
      ...
      if [ -n "$prev_line" ]; then
        upload_mbps=$(echo "$prev_line" | awk '{print $1}')
        if [ -n "$upload_mbps" ] && [ "$upload_mbps" != "0" ]; then
          echo "Up: ↑ ${upload_mbps}MB/s"
          exit 0
        fi
      fi
  ...
  if command -v speedtest-cli >/dev/null 2>&1; then
    ...
    if [ -n "$test_result" ]; then
      ...
      if [ -n "$upload_mbps" ] && [ -n "$download_mbps" ]; then
  ...
  if command -v fast-cli >/dev/null 2>&1; then
    # fast-cli only tests download, but we can use it as fallback
    # For upload, we'd need a different service
    test_result=$(fast-cli --json 2>/dev/null)
    
    if [ -n "$test_result" ]; then
      # fast-cli typically only provides download speed
      # For now, set upload to 0 or use a placeholder
      upload_mb="0.0"
